...
from src.config import (CITIES, PROCESSED_DIR, PATCH_SIZE_M)
...
def download_city_buildings(city_name):
    """Download all buildings for a city."""
    print(f"  downloading buildings...")
    try:
        gdf = ox.features_from_place(
            city_name, tags={'building': True}
        )
...
def download_city_open_space(city_name):
    """
    Download all public open space for a city in one call.
    Combines parks, grass, and pedestrian areas.
    """
    print(f"  downloading open space...")
    all_features = []

    # query each tag type separately
    tag_queries = [
        {'leisure': 'park'},
        {'landuse': 'grass'},
        {'highway': 'pedestrian'},
    ...
    # combine all open space into one geodataframe
