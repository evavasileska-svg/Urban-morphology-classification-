...
from src.config_global import (CITIES, RAW_DIR, PROCESSED_DIR,
...
def compute_graph_metrics(G, patch_size_m):
    """Compute graph metric features from one patch subgraph."""

    # Convert directed OSM graph into undirected physical street graph
    G_u = ox.convert.to_undirected(G)

    # Count physical nodes
    n_nodes = G_u.number_of_nodes()

    # Use OSMnx street counts for realistic street connectivity
    # This avoids counting two-way streets twice
    street_counts = ox.stats.count_streets_per_node(G)
    degrees = list(street_counts.values())

    if not degrees:
        return None

    # Edge length features from undirected graph
    edge_lengths = [
        data.get('length', 0)
        for _, _, data in G_u.edges(data=True)
    ]
    ...
    # Count physical street segments after filtering valid lengths
    ...
    cv_edge_length = (
        std_edge_length / mean_edge_length
    ...
    grain_ratio = (
        mean_node_degree / mean_edge_length
        if mean_edge_length > 0 else 0
    )

    meshedness = (
        (n_edges - n_nodes + 1) / (2 * n_nodes - 5)
        if n_nodes > 3 else 0
    )
